...
function pinCompleteBookmark(context, args){
  ...
  context.title = ["Page URL"];
        ...
        context.completions = frames.map( (win) => [win.document.documentURI, frames.length == 1 ? "Current Location" : "Frame: " + win.document.title]);
  // context.completions = [[win.document.documentURI, frames.length == 1 ? "Current Location" : "Frame: " + win.document.title] for ([, win] in Iterator(frames))];
  return;
    }
};
